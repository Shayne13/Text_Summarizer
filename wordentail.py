...
#         vector_func=randvec,        # Should be a map from the strings in vocab to vectors (e.g., randvec).
#         vector_combo_func=vec_concatenate): # Use vec_concatenate or write something better!
#     # Load in the dataset:
...
#                 item = (vector_combo_func(vectors[w1], vectors[w2]), [clsname])
#                 #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
